...
- 历史持久化到应用数据目录（`pier-core::paths::data_dir()` 下 `ai-history/`），与 smart-mode history 同一落盘策略；设置里可切到「仅内存」。每个会话有清空按钮。应用重启后，前端从 transcript 回放可见消息，后端在该 tab 首次发送前把同一 transcript 恢复成模型上下文；超过窗口的旧记录本地压缩成一条上下文恢复提示，仅保留最近可用的对话和工具结果，避免 UI 可见但模型“失忆”。
- 会话隔离粒度是 **terminal tab**，不是全局 AI 线程：不同 tab 的 AI 面板、transcript、会话临时许可互不共享；同一 tab 内的 SSH 重连 / cwd 变化仍沿用该 tab 的 AI 会话。若未来要做到“同一 tab 内每次 terminal session 强隔离”，必须先把本条从 per-tab 改为 per-terminal-session，并处理旧 transcript 迁移。
